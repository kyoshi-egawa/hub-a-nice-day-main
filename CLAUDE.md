...
- **バージョン番号**: 本番反映を伴う機能追加・修正のたびに `index_dev.html` の `APP_VERSION` を0.01上げる（モバイルの変更は `mobile.html` の `MOBILE_VERSION`）。緊急修正のみ・本番反映なしの実験は上げなくてよい。ヘッダーのバージョン表示はユーザーが「どの版が配信されているか」を確認する手段なので、上げ忘れないこと。
- 日本語コミットメッセージで可。

...
- **バージョン番号はシステム全体で1つ**: 機能追加・修正を反映するたびに0.01上げ、`index_dev.html` の `APP_VERSION` と `mobile.html` の `MOBILE_VERSION` を**必ず同じ値に揃える**（どちらか一方だけの変更でも両方上げる）。ヘッダーのバージョン表示はユーザーが「どの版が配信されているか」を確認する手段なので、上げ忘れないこと。
- 日本語コミットメッセージで可。
